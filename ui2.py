"""
ESP32-CAM  ·  Assistive Text Reader  ·  v2.1  (J.A.R.V.I.S. Edition)
══════════════════════════════════════════════════════════════════
✦ High-Res Window Resizing
...
# ── Config ─────────────────────────────────────────────────────────────────────
IP            = "192.168.1.204"  # Change to your ESP32's IP
...
ALPHA_BOX   = 0.15
SIDE_W      = 320              
HUD_FONT    = cv2.FONT_HERSHEY_COMPLEX_SMALL 
...
def draw_label_chip(canvas, text, x1, y1, color):
    ...
    cv2.line(canvas, (lx, ly+bl+py), (lx+tw+px*2, ly+bl+py), color, 1, cv2.LINE_AA) 
...
def draw_side_panel(canvas, sentence, detections, h, w, n_spoken, is_active):
    px = w - SIDE_W
    ...
    if not is_active:
        txt("SYSTEM SUSPENDED", C_DIM, 0.55)
    elif sentence:
    ...
    controls = "[Q] QUIT  [O] OPTICS  [S] EXPORT  [P] POWER  [R] REPEAT"
    cv2.putText(canvas, controls, (px+10, h-15), HUD_FONT, 0.45, C_DIM, 1, cv2.LINE_AA)

def draw_bottom_bar(canvas, sentence, h, w, is_active):
    blend_rect(canvas, 0, h-28, w - SIDE_W, h, C_PANEL, ALPHA_PANEL)
    cv2.line(canvas, (0, h-28), (w-SIDE_W, h-28), C_MAIN, 1)
    
    if not is_active:
        sentence = "STANDBY MODE ACTIVE"
        color = C_DIM
    elif sentence:
        danger = is_danger(sentence)
        color  = C_DANGER if danger else C_ACCENT
...
def main():
    ...
    # Set up the window BEFORE the loop so it's massive
    window_name = "MARK II // OPTICS"
    cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
    cv2.resizeWindow(window_name, 1280, 720) # 720p minimum footprint
    ...
    # State flags
    show_original = True
    system_active = True  # Handles the new [P] Power Toggle
    ...
    while True:
        # We must keep reading frames even in standby to prevent buffer lag
        ret, frame = cap.read()
        if not ret:
        ...
        if system_active:
            ...
            # Big Standby Text
            standby_txt = "SYSTEM SUSPENDED"
            (tw, th), _ = cv2.getTextSize(standby_txt, HUD_FONT, 1.5, 2)
            cv2.putText(canvas, standby_txt, ((w - tw)//2, h//2), HUD_FONT, 1.5, C_DIM, 2, cv2.LINE_AA)
            cv2.putText(canvas, "PRESS [P] TO INITIALIZE OPTICS", ((w - 320)//2, h//2 + 40), HUD_FONT, 0.8, C_ACCENT, 1, cv2.LINE_AA)

        # ── Global Panels (Draw these whether active or standby) ──────────────
        draw_top_bar(canvas, mode_tag, len(detections), fps if system_active else 0, total_w)
        draw_side_panel(canvas, sentence, detections, h, total_w, n_spoken, system_active)
        draw_bottom_bar(canvas, sentence, h, total_w, system_active)
        ...
        if   key == ord('q'): break
        elif key == ord('o') and system_active: 
            ...
            if not system_active and pygame.mixer.music.get_busy():
                pygame.mixer.music.stop()
                pygame.mixer.music.unload()
        elif key == ord('r'):  
            if system_active and last_text:
                print(f"[SYSTEM] Repeating Audio: {last_text}")
                threading.Thread(target=speak, args=(last_text,), daemon=True).start()
